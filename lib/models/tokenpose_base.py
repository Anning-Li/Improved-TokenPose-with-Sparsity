...
class Attention_w_mask(nn.Module):
    ...
    def forward(self, x, mask, attn_keep_ratio=1.0, attn_prune=False):
        ...
        if attn_prune:
            ...
            top_val, _ = x.topk(dim=2, k=num_keep_attn)  # (B, HW, HW * attn_keep_ratio)
            mask_img = (x >= top_val[:, :, -1].unsqueeze(-1).expand(-1, -1,
                                                                    n1)) + 0  # （B, HW, HW） without gradient     0/1 mask
...
class TokenPose_S_base(nn.Module):
    def __init__(self, *, image_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, joint_mask,
        ...
        self.joint_transformer = Transformer(dim, depth, heads, mlp_dim, dropout, num_keypoints=num_keypoints,
                                             all_attn=self.all_attn)
    ...
    def forward(self, joint_mask, img, mask):
        p = self.patch_size
        # stem net
        x = self.conv1(img)
        x = self.bn1(x)
        ...
        mask = attn_mask = torch.ones(b, n, n).to(x.device)
        x = self.transformer(x, self.pos_embedding, mask=attn_mask)
...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False,
        ...
        # transformer
        self.transformer = Transformer(dim, depth, heads, mlp_dim, dropout, mask=None,
                                       num_keypoints=num_keypoints,
                                       all_attn=self.all_attn)
        self.joint_transformer = Transformer(dim, depth, heads, mlp_dim, dropout, mask=None,
                                             num_keypoints=num_keypoints,
                                             all_attn=self.all_attn)
...
class TokenPose_L_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False,
                 ...
                 emb_dropout=0., pos_embedding_type="learnable", apply_multi=True):
        super().__init__()
        assert isinstance(feature_size, list) and isinstance(patch_size,
                                                             list), 'image_size and patch_size should be list'
        assert feature_size[0] % patch_size[0] == 0 and feature_size[1] % patch_size[
